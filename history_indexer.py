...
def get_docs(df):
    ...
    for _, row in df.iterrows():
        ...
        clean_df.drop(['bathrooms', 'neighbourhood_group_cleansed', 'calendar_updated', 'license', 'description', 'neighborhood_overview', 'host_url', 'host_name', 'host_location', 'host_about', 'picture_url', 'host_thumbnail_url', 'host_picture_url', 'host_neighbourhood', 'host_has_profile_pic', 'host_identity_verified', 'neighbourhood', 'latitude', 'longitude', 'property_type', 'room_type', 'bathrooms_text', 'bedrooms', 'beds'], axis=1, inplace=True, errors='ignore')
    ...
    try:
        ...
        for file in os.listdir(path):
            if file.endswith(".csv.gz"):
                ...
                clean_df[['listing_url', 'name', 'host_response_time', 'host_response_rate', 'host_acceptance_rate', 'host_is_superhost', 'neighbourhood_cleansed', 'price', 'first_review', 'last_review']] = clean_df[['listing_url', 'name', 'host_response_time', 'host_response_rate', 'host_acceptance_rate', 'host_is_superhost', 'neighbourhood_cleansed', 'price', 'first_review', 'last_review']].fillna(value=" ")

                # fill NaN with 0.0 for float values
                clean_df[['review_scores_rating', 'review_scores_accuracy', 'review_scores_cleanliness', 'review_scores_checkin', 'review_scores_communication', 'review_scores_location', 'review_scores_value', 'reviews_per_month']] = clean_df[['review_scores_rating', 'review_scores_accuracy', 'review_scores_cleanliness', 'review_scores_checkin', 'review_scores_communication', 'review_scores_location', 'review_scores_value', 'reviews_per_month']].fillna(value=0.0)

                # fill NaN with 0 for integer values
                clean_df[['minimum_nights', 'maximum_nights', 'maximum_minimum_nights', 'minimum_minimum_nights', 'maximum_minimum_nights', 'minimum_maximum_nights', 'maximum_maximum_nights', 'minimum_nights_avg_ntm', 'maximum_nights_avg_ntm']] = clean_df[['minimum_nights', 'maximum_nights', 'maximum_minimum_nights', 'minimum_minimum_nights', 'maximum_minimum_nights', 'minimum_maximum_nights', 'maximum_maximum_nights', 'minimum_nights_avg_ntm', 'maximum_nights_avg_ntm']].fillna(value=0)

                docs = get_docs(clean_df)
